...
def RC_d(R0, C0):
    ...
    Parameters:
        R0 (int) : the population density of resource population at time t
        C0 (int) : the population density of consumer population at time t
    ...
    for i in range(0, len(t)-1):
        RC[i+1][0] = RC[i][0] * (1 + ((r  + ER) * (1 - RC[i][0] / K)) - a * RC[i][1])
        RC[i+1][1] = RC[i][1] * (1 - z + EC + e * a * RC[i][0])
        if RC[i+1][0] > K:
            RC[i+1][0] = K
        elif RC[i+1][0] < 0:
            RC[i+1][0] = 0
...
K = 30
EC = sc.random.normal(0, 1)
ER = sc.random.normal(0, 1)
...
# run the function on our initial population densities
RC = list(RC_d(R0, C0))
R = [x[0] for x in RC]
C = [x[1] for x in RC]
